...
function ReportActions({ closeState }) {
    return (
        <div className="w-full no-print h-[56px] bg-gray-500 flex items-center justify-between px-4 shadow-md ">
            ...
            <button onClick={() => window.print()} className="bg-gray-50 p-1.5 px-4 rounded-md shadow-md font-semibold duration-150 hover:bg-gray-200 flex items-center gap-x-1">
                {<icons.Print />}
                Print
            </button>
        </div>
